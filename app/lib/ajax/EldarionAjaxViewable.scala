...
trait AjaxModule extends EldarionAjaxResponseEncodings {
  ...
  case class ViewableModel(it: AnyRef, view: String, javascript: String, args: (String, Any)*) extends AjaxViewable  { 
    ...
    def renderHtml: Frag = renderModelView(it, view, args: _*)
  }

  implicit class NodeToAjaxViewable(n:Frag) extends AjaxViewable {
    def toJson(): JsValue = Json.toJson(Map("html"->n.toString))
  }
  implicit class StringToAjaxViewable(s:String) extends AjaxViewable {
    def toJson(): JsValue = Json.toJson(Map("html"->s))
  }
  implicit class NumericToAjaxViewable[T: Numeric](u: T) extends AjaxViewable {
    def toJson(): JsValue = Json.toJson(Map("html"->u.toString))
  }
  ...
  case class SessionMessageResult(result: Result) {
    def appendSession(key: String, value: String)(implicit request: Request[_]):Result = result.withSession(request.session + (key -> value))
    ...
    def clearSessionMessages(implicit request: Request[_]): Result = {
    ...
    private def withStickySessionMessage(f: SessionMessageType, value: String)(implicit request: Request[_]): Result = {
      val current = request.session.data.filterKeys(_.startsWith("sessionmessage-"))
    ...
    def success(value: String)(implicit request: Request[_]): Result = withStickySessionMessage(Success, value)
   
    def info(value: String)(implicit request: Request[_]): Result = withStickySessionMessage(Info, value)
   
    def warning(value: String)(implicit request: Request[_]): Result = withStickySessionMessage(Warning, value)
   
    def error(value: String)(implicit request: Request[_]): Result = withStickySessionMessage(Error, value)
  }
  ...
  object SessionMessage {
    ...
    implicit def toRichResult[T <: Result](result: T) = SessionMessageResult(result)
...
trait EldarionAjaxViewable {
  def toJson(m: Map[String, String]): JsValue = new JsString("FIXME toJson")
}
/**
 ...
 */
case class EldarionAjaxViewableModel(html: Frag, javascript: String, args: (String, Any)*) extends EldarionAjaxViewable  { 
...
case class EldarionAjaxErrorViewable(message: String, javascript: String, args: (Symbol, Any)*) extends EldarionAjaxViewable { //extends EldarionAjaxResponseElement {
  ...
  // FIXME def renderJs(): JsValue = toJson(Map("error" -> ScalateOps.viewHtml(it)(args: _*), "js" -> javascript))
  def renderJs(): JsValue = toJson(Map("error" -> "FIXME Some Error msg here ", "js" -> javascript))
}
